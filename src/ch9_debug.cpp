//
// Created by m0dulo on 2022/12/11.
//
...
static std::unique_ptr<ExprAST> ParseBinOpRHS(int ExprPrec,
  ...
  while (true) {
    ...
    LHS =
        std::make_unique<BinaryExprAST>(BinLoc, std::move(LHS), std::move(RHS));
  }
}
...
Value *ForExprAST::codegen() {
  ...
  // VarName); Variable->addIncoming(StartVal, PreheaderBB);
...
static void HanldeTopLevelExpression() {
  if (auto FnAST = ParseTopLevelExpr()) {
    if (!FnAST->codegen()) {
      ...
      // RT)); InitializeModuleAndPassManager();
